...
public class WaypointManager {
    ...
    public static void loadAll() {
        ...
        for (File file : files) {
            try {
                ...
                CarpetPlayersPlugin.logError("Gagal load waypoint: " + file.getName(), e);
            }
        }
    }
    ...
    public static void savePlayer(UUID uuid) {
        ...
        try (Writer writer = new FileWriter(file)) {
            ...
            CarpetPlayersPlugin.logError("Gagal simpan waypoint untuk " + uuid, e);
        }
    }
     ...
     * Logic: Setiap kali mati, "Death" lama dihapus, "Death" baru dibuat.
     * "Old Death" (waypoint mati kedua) dibuat otomatis.
     * Pada kematian ketiga, "Old Death" dihapus.
     */
    public static void handleDeath(UUID uuid, Location loc) {
        List<Waypoint> list = getWaypoints(uuid);
        String worldName = loc.getWorld() != null ? loc.getWorld().getName() : "world";

        // Hapus "Death" jika ada
        list.removeIf(w -> w.name.equalsIgnoreCase("Death"));

        // Cek "Old Death" — jika sudah ada, hapus (kematian ke-3 dst)
