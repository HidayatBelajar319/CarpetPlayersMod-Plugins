...
public class BotBrain {
    ...
    public String aiGetStateInfo() {
        try {
            ...
            String dim = bot.getWorldServer().getDimensionKey().a().toString();
            String hand;
            try {
                ...
                if (main == null || main.isEmpty()) {
                    ...
                    hand = main.getName().getString();
                    if (hand == null || hand.isEmpty()) {
                        hand = IRegistry.ITEM.getKey(main.getItem()) != null
                                ? IRegistry.ITEM.getKey(main.getItem()).toString()
                                : "unknown";
                    }
                }
            } catch (Exception e) {
    ...
    public void aiAttack(String targetName) {
        ...
        EntityPlayer player = bot.getServer().getPlayerList().getPlayer(targetName);
        if (player == null) {
            return;
        }
        setTarget(player);
    ...
    public void aiMineAt(int x, int y, int z) {
        ...
        WorldServer level = bot.getWorldServer();
        if (level == null || !level.isLoaded(pos)) {
            return;
        }
        IBlockData state = level.getType(pos);
        if (state.isAir()) {
            return;
        }
        ...
        if (state.getDamage(bot, level, pos) < 0.0F) {
            return;
        }
        bot.lookAt(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5);
        bot.playerInteractManager.breakBlock(pos);
    }

    public void aiMineNearest() {
        BlockPosition mine = findMineableBlock();
        if (mine == null) {
            return;
        }
        bot.lookAt(mine.getX() + 0.5, mine.getY() + 0.5, mine.getZ() + 0.5);
        bot.playerInteractManager.breakBlock(mine);
    }

    public void aiUseItem() {
        bot.playerInteractManager.a(bot, bot.getWorldServer(), bot.getItemInMainHand(), EnumHand.MAIN_HAND);
    }
    ...
    protected Entity findTarget() {
        ...
        AxisAlignedBB box = bot.getBoundingBox().grow(radius);
        List<EntityPlayer> players = bot.getWorldServer().getEntities(EntityPlayer.class, box,
                p -> p != bot && p.isAlive());
        Entity nearest = null;
    ...
    protected void tickFollow() {
        ...
        if (distanceSq > 9.0) {
            bot.lookAt(owner.locX(), owner.locY(), owner.locZ());
            setMovement(1.0F, 0.0F);
        } else {
            setMovement(0.0F, 0.0F);
        }
    ...
    protected void manageWeapon() {
        ...
        if (best >= 0 && best != bot.inventory.itemInHandIndex) {
            bot.inventory.itemInHandIndex = best;
        }
    }
    ...
    protected void tryEat() {
        ...
        FoodMetaData foodData = bot.getFoodData();
        ...
        bot.inventory.itemInHandIndex = slot;
        bot.startUsingItem(EnumHand.MAIN_HAND);
        eatCooldown = 200;
    }
    ...
    protected void tryUseMilk() {
        ...
        boolean hasBadEffect = bot.getActiveEffects().stream()
                .anyMatch(effect -> effect.getMobEffect() instanceof net.minecraft.server.v1_16_R3.MobEffectList
                        && isHarmful(effect));
        if (!hasBadEffect) {
            return;
        }
        for (int i = 0; i < 9; i++) {
            if (bot.inventory.getItem(i).getItem() == Items.MILK_BUCKET) {
                bot.inventory.itemInHandIndex = i;
                bot.playerInteractManager.a(bot, bot.getWorldServer(), bot.getItemInMainHand(), EnumHand.MAIN_HAND);
                return;
            }
        }
    ...
    protected boolean isHarmful(net.minecraft.server.v1_16_R3.MobEffect effect) {
        try {
            return !effect.getMobEffect().getClass().getSimpleName().contains("Beneficial")
                    && effect.getMobEffect().getColor() != 0;
        } catch (Exception e) {
            return true;
        }
    }
    ...
    protected boolean hasArrows() {
        for (int i = 0; i < bot.inventory.getSize(); i++) {
    ...
    protected int findBestFoodSlot() {
        ...
        for (int i = 0; i < 36; i++) {
            ItemStack stack = bot.inventory.getItem(i);
            if (stack.isEmpty() || !stack.getItem().isFood() || stack.getItem() == Items.POTION
                    || stack.getItem() == Items.SPLASH_POTION || stack.getItem() == Items.LINGERING_POTION) {
                continue;
            }
    ...
    protected void broadcastBot(String message) {
        ...
        list.sendMessage(new ChatComponentText("<" + getBotName() + "> " + message), ChatMessageType.CHAT,
                bot.getUniqueID());
    }
    ...
    protected void pickWanderTarget() {
        WorldServer level = bot.getWorldServer();
        BlockPosition pos = bot.blockPosition();
        ...
        for (int i = 0; i < 8; i++) {
            ...
            BlockPosition candidate = new BlockPosition(pos.getX() + dx, pos.getY(), pos.getZ() + dz);
            if (safeSpot(level, candidate)) {
                wanderTarget = new Vec3D(candidate.getX() + 0.5, candidate.getY(), candidate.getZ() + 0.5);
                return;
            }
        }
        wanderTarget = null;
    ...
    protected boolean safeSpot(WorldServer level, BlockPosition pos) {
        BlockStatePos below = stateAt(level, pos.down());
        if (below.isLavaFire) {
            return false;
        }
        ...
        if (stateAt(level, pos.up()).isSolid) {
            return false;
        }
        return true;
    }
    ...
    protected boolean hazardAhead() {
        BlockStatePos below = stateAt(bot.getWorldServer(), bot.blockPosition().down());
        return below.isLavaFire;
    }

    protected BlockStatePos stateAt(WorldServer level, BlockPosition pos) {
        Material material = level.getType(pos).getMaterial();
    ...
    protected BlockPosition findMineableBlock() {
        WorldServer level = bot.getWorldServer();
        BlockPosition pos = bot.blockPosition();
        for (int dx = -2; dx <= 2; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dz = -2; dz <= 2; dz++) {
                    ...
                    BlockPosition p = pos.b(dx, dy, dz);
                    if (p.equals(pos.down())) {
                        continue;
                    }
                    IBlockData state = level.getType(p);
                    if (state.isAir()) {
                        continue;
                    }
                    Material material = state.getMaterial();
                    ...
                    if (state.getDamage(bot, level, p) < 0.0F) {
                        continue;
                    }
                    return p;
                }
    ...
    public void tickControlled(EntityPlayer controller) {
        bot.lookRotation(controller.yaw, controller.pitch);
        Vec3D pos = controller.getPositionVector();
        if (controllerLastPos != null) {
            double dx = pos.x - controllerLastPos.x;
            double dz = pos.z - controllerLastPos.z;
            float yawRad = (float) Math.toRadians(controller.yaw);
        ...
        float scale = controller.getAttackCooldown(1.0F);
